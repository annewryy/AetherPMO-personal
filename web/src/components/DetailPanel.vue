...
// CatalogDetailPanel의 마스터-디테일 패턴을 계승하되, 이 패널은 목록 위에 겹치는 드로어다.
//  - 헤더: displayCode + 제목 + 현재 상태 뱃지
//  - 필드: 담당자·목표해결일·우선순위·상세내용 등 — 도메인별 세트, 인라인 PATCH(쓰기 게이트)
//  - 상태 전이:
//      · 이슈/액션 = 상태 사다리(발생→조치중→완료, 완료→조치중 재오픈 등) — 가능한 것만 노출.
//        상태 변경 시 사유 코멘트(선택) 모달.
//      · 산출물/태스크 = 워크플로 엔진 GET transitions(TransitionButtons 재사용).
//  - 첨부파일: 아마란스 위임 stub 링크.
//  - 하단: 코멘트 스레드(C-2 — 답글·@멘션·알림).
...
.dr-head-main { display: flex; align-items: center; gap: 8px; }
.kind-chip {
...
.dr-title { font-size: 17px; margin: 0; word-break: break-word; }
.dr-status-row { display: flex; align-items: center; gap: 8px; }
.plain-status {
...
.f-area { resize: vertical; width: 100%; }
