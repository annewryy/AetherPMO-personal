...
.row { display: flex; align-items: center; gap: 8px; padding: 6px 8px; font-size: 13px; border-radius: 6px; }
.row.clickable { cursor: pointer; }
.row.clickable:hover { background: var(--panel-2); }
.row.selected { background: rgba(139, 92, 246, 0.22); outline: 1px solid var(--accent); }
.caret { color: var(--muted); font-size: 11px; transition: transform 0.12s; width: 12px; flex-shrink: 0; cursor: pointer; }
...
.code { font-family: ui-monospace, monospace; font-size: 11px; color: var(--muted); flex-shrink: 0; }
.name { min-width: 0; }
.optional {
...
.inactive {
  flex-shrink: 0; font-size: 10px; color: var(--red);
  border: 1px solid rgba(239, 68, 68, 0.5); border-radius: 999px; padding: 0 6px;
}
.dcount { flex-shrink: 0; margin-left: auto; font-size: 10px; color: var(--muted); }
