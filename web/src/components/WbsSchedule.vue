...
const rows = computed<FlatRow[]>(() => {
  ...
  for (const phase of props.wbs.phases ?? []) {
    out.push({ node: phase, depth: 0 });
    for (const activity of phase.activities ?? []) {
      out.push({ node: activity, depth: 1 });
      for (const task of activity.tasks ?? []) {
        out.push({ node: task, depth: 2 });
      }
    }
  }
  return out;
...
function bar(node: WbsNode): Bar | null {
  ...
  // 지연 판정: 완료(DONE/100%)가 아니면서 오늘이 계획 종료를 지났거나,
  //   목표 대비 실제가 뒤처진(delta<0) 경우 빨강. (기대치 없으면 종료 초과만으로 판정.)
  const done = node.actualRate >= 100 || node.status === 'DONE';
  const overdue = todayDay > end && !done;
  const behind = node.delta != null && node.delta < 0;
  const delayed = !done && (overdue || behind);
...
<template>
  ...
  <div v-else class="wbs">
    ...
    <div class="legend">
      ...
      <span class="lg"><span class="sw sw-delayed" /> 지연</span>
...
.wbs-head, .wbs-row {
  ...
  grid-template-columns:
    minmax(220px, 1.6fr)  /* WBS 트리 */
    100px                 /* 담당자 */
    76px                  /* 상태 */
    56px 56px 52px        /* 목표%·실제%·Δ */
    minmax(280px, 2.4fr); /* 간트 */
  align-items: center;
}
...
@media (max-width: 1100px) {
  .wbs-head, .wbs-row {
    grid-template-columns: minmax(180px, 1.4fr) 84px 68px 48px 48px 44px minmax(220px, 2fr);
  }
}
</style>
