...
status: A 구현 완료(2026-07-14, V14) · B 대기
...
## A. 누락 필드 (공통 화면 — 유경님 O / 우리 X) — **✅ 구현 완료(V14, impl/0013 1dc1685)**

작고 확실한 갭. 기존 테이블에 컬럼 추가 + 엔티티·매퍼·쓰기·프론트 배선 완료.
검증: participation_rate·agenda·decisions·description API 왕복, 참여율 폼/목록 스크린샷.
(컨소시엄 contact_*는 컬럼+읽기만 — 컨소시엄 편집 화면이 아직 없어 쓰기 경로는 후속.)
