...
## J. 개발서버 NAS 실사 결과 (2026-08-04 — 하단 "미결 3개" 부분 해소)

- **벤더 = Synology** (모델 미확인 — DSM 제어판 > 정보 센터에서 확인 필요), 주소 `192.168.0.100`(DSM :5000/:5001).
- **열린 서비스**: DSM(5000/5001)·SMB(445/139)만. **NFS(2049)·WebDAV·S3(9000) 꺼짐** — 현재 파일 프로토콜은 SMB뿐.
- **Synology는 자체 S3 서버 기능 없음**(C2 Object Storage는 클라우드 서비스라 해당 없음). S3 호환을 쓰려면
  **Container Manager(Docker)에 MinIO 설치**(Docker 지원 모델 + DSM 관리자 권한 필요).
- **⛔ 결정적 차단**: 개발서버(172.30.1.22, `172.30.1.0/24`) → NAS(`192.168.0.x`) **경로 없음**(ping·전 포트
  불통, 2026-08-04 실측). 사내망 PC에서는 접근 가능. **백엔드가 개발서버에서 돌므로 이 경로가 뚫리기 전엔
  어떤 어댑터로도 이 NAS 사용 불가.** → 인프라팀에 방화벽/라우팅 오픈 요청(SMB 445 + MinIO 9000 둘 다).

### 아키텍처 옵션 (경로 오픈 후 확정)
| 옵션 | 방법 | 조건 | presigned 직접 업로드 |
|---|---|---|---|
| A. NAS에 MinIO | Container Manager에 MinIO, S3Adapter가 NAS:9000 사용 | Docker 지원 모델 + DSM 관리자 권한 | 가능(브라우저→NAS) |
| B. SMB 마운트 | 개발서버에 CIFS 마운트 + NfsAdapter(스트리밍) | 공유폴더 권한만 | 불가(앱 경유) |
| A'. 개발서버 MinIO + NAS 볼륨 | compose MinIO의 데이터 볼륨 = NAS CIFS 마운트 | 공유폴더 권한만 | 가능(브라우저→개발서버) |
...
- ~~NAS 정체 3개 확인~~ → **§J로 해소**(벤더 Synology·S3 미지원(MinIO 설치 시 가능)·사내망 접근 가능).
  잔여: 모델명(Docker 지원 여부), DSM 관리자 권한 여부, **개발서버→NAS 방화벽 오픈**(인프라팀).
