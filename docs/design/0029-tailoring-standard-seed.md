...
## Phase B 구현 — 유형·규모 연동 자동 전개 (2026-07-26, impl/0035)

수행 전환 마법사(ExecConvertWizard) Step2에 적용:
- **사업 유형** 칩: SI 구축(OPMS+ODS) / 유지보수·운영(OPMS+OMS) / ISP·컨설팅(OPMS+BIS) / 전체(커스텀 포함).
  선택 시 트리를 해당 방법론 루트로 필터, 유형 변경 시 필터 밖 선택은 자동 해제.
- **규모 판정**: Step1 계약금액으로 자동 — 10억↓ 소 / 10~50억 중 / 50억↑ 대(required_* 열 기준).
- **규모별 필수 자동 선택** 버튼: 판정된 규모의 필수 산출물(required_{small|medium|large}=1)을
  일괄 선택("필수 N건 중 M건 선택됨" 표시). 선택 산출물은 수동 추가. 조상 전개는 기존 로직 재사용.
- 잔여(후속): 나라장터→입찰 마법사에도 동일 컨트롤 적용, Phase C(테일러링 전용 마법사) 시 통합.
