...
- **아마란스 필수 게이트**: 아마란스에서만 가능한 기능(결재 실행 등)만 연동 없으면 비활성. → 헥사고날
  Port로 격리(`ApprovalPort` = 아마란스 필수, `OrgDirectoryPort` = 아마란스 동기화 어댑터 + 수동 기본,
  `AuthenticationPort` = 로컬 로그인 기본 — 종전 `UserPort`를 로그인/동기화 두 seam으로 분리, 0013 §B-2).
...
## H. 이원 로그인 모델 (2026-07-26 확정 — 너울님)

로그인 경로를 **두 가지**로 확정한다:

1. **아마란스 연동 로그인** (자사 직원, 연동 스펙 확보 후 활성):
   - 아마란스 인증으로 로그인하고 **연동 키를 세션에 보관**(`pms_session.amaranth_key` — 스펙 확보 시 컬럼 추가).
   - 키 보유 세션만 **아마란스 위임 기능**(전자결재 확인 등) 활성. 키 없으면 해당 기능만 비활성(자체완결 원칙).
   - 연동 가능 여부 = `pms_org_member` 매칭(로그인 응답 `amaranthLinked`) — 기능 게이트 표시용.
   - **선행 조건(미확보)**: 아마란스 인증 API/키 발급 스펙 — 확보 전까지 로그인 화면에 "준비중" 안내.
2. **자체 비밀번호 로그인** (외부 인력 + 아마란스 연동이 안 되는 자사 직원):
   - 현행 PBKDF2 경로. **아마란스 위임 기능만 비활성**되고 나머지는 동일하게 동작.
   - 비밀번호 미설정 계정(아마란스 동기화 897명)은 로그인 시 안내: 연동 준비중 / 관리자 발급 요청.
   - 관리자 발급: `POST /api/admin/users/password` (SYS_ADMIN 전용, RBAC 가드) — 사용자 관리 화면은 후속.

## I. 인가(Authorization) 경계 (2026-08-04 확정 — 솔루션화 원칙)

- 외부 시스템(아마란스·고객사 AD 등)에서 가져오는 것은 **신원·조직 데이터까지**: 누구인가(로그인
  `AuthenticationPort`), 어느 부서인가(`OrgDirectoryPort`). 0013 §B-2 포트 규율과 동일.
- **"이 사용자가 이 리소스에 접근 가능한가"라는 인가 판단은 항상 PMS 내부 모델이 수행**:
  `pms_user.role`(RBAC 5종, 0041) + 프로젝트 멤버/역할(`pms_project_member`). 외부 위임 금지.
- 이유: 인가까지 외부 의존하면 뒤 시스템이 바뀔 때마다 권한 체계가 흔들리고, 고객사마다 권한
  시스템 연동을 요구하게 됨. 신원은 갈아끼우되 권한 규칙은 제품이 소유해야 솔루션화가 성립.
- 파일 접근(0018)도 동일: NAS는 앱 서비스 계정 1개만 알고, 파일별 접근 판단은 백엔드 API가
  entity 권한(프로젝트 멤버십)으로 매 요청 검증.
