...
## A. 나라장터 인앱 상세 — **UX 재설계(2026-07-09 확정)**
초기 구현(P2 batch10)은 우측 **드로어 인라인 폼**이었으나 "낡았고, 항목 많으면 드로어에 안 맞음" 피드백 →
현대 SaaS 정석으로 재구성:
- **리스트 → 상세 = 전체 페이지 전환**(라우트 `/app/bid-notices/{bidNtceNo}`, "목록으로" 뒤로). 드로어 폐기.
  콘텐츠 많은 상세는 드로어(요약용)보다 **전용 페이지**가 정석(드로어 스크롤 회피).
- **풀필드 = `inqryDiv=2` 단건조회 확정**: 신규 `GET /api/bid-notices/{bidNtceNo}`(batch14) — 계약방법·낙찰방법·
  추정가격/배정예산·지역/업종제한·담당자·공고규격서URL 등 리치 필드. 상세 페이지에 섹션 구성.
- **등록 = 상세 페이지의 버튼 → 중앙 모달 3스텝 마법사**(§C 참조). 인라인 폼 아님.
- 역할 분리: 리스트=훑기, 상세=전체 페이지 읽기, 등록=모달 집중 작업.

> 초기 8필드(목록)로 만든 드로어 폼(batch10/12/13)은 이 재설계로 대체 예정. 기능(프리필·테일러링·생성)은 보존, UI만 재구성.
...
### C-2. 등록 = 중앙 모달 3스텝 마법사 (2026-07-09 재설계)
인라인 폼(P2) 폐기. 상세 페이지의 "입찰 프로젝트 등록" → **딤 배경 중앙 모달**(≈820~900px) + 스텝 인디케이터:
- **Step 1. 기본정보** — 공고에서 프리필된 필드(사업명·고객·계약금액·공고번호·마감일 등) 편집.
- **Step 2. 카탈로그 선택** — §C-1 선택기(검색·필터·미리보기, cascade up/down).
- **Step 3. 확인 & 생성** — 입력값 + 선택 항목 수 요약 → `POST /api/projects`(tailoring 동봉).
- 이전/다음/생성, 모던 스타일(여백·위계). 기능 보존, UI만 재구성.

## 단계 구현 — **P1~P3 완료(2026-07-09), dev 검증됨. 이후 UX 재설계 진행중**
